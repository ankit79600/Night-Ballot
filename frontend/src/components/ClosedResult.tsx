...
import { ShareBallot } from './ShareBallot';
...
export function ClosedResult({ ballotState, contractAddress }: Props) {
  ...
  const winnerColor =
    ballotState.yesVotes > ballotState.noVotes ? 'text-emerald-400'
    : ballotState.noVotes > ballotState.yesVotes ? 'text-red-400'
    : 'text-white';
  ...
  return (
    <div className="rounded-2xl border border-white/[0.08] bg-white/[0.02] p-8">
      ...
      <div className={`text-[48px] font-black tracking-tight mb-1 ${winnerColor}`}>{winner}</div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <ExportResults ballotState={ballotState} contractAddress={contractAddress} />
      </div>
    </div>
  );
}
